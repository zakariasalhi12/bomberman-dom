...
export class Game {
    ...
    joinPlayerToRoom(playerData, socket) {
        console.log('Player joining room:', playerData);
        ...
        if (room.players.size >= 2) {
            ...
            if (room.players.size === MAX_PLAYERS) {
                console.log('Max players reached, starting game immediately');
    ...
    startCountdown(room) {
        console.log('Starting countdown for room:', room.id);
        if (room.state !== GAME_STATES.WAITING) {
            console.log('Cannot start countdown - room state is:', room.state);
            return;
        }
        ...
        console.log('Broadcasting countdown message:', countdownMessage);
        this.broadcastToRoom(room, countdownMessage);
    ...
    startGame(room) {
        ...
        if (room.state !== GAME_STATES.COUNTDOWN) {
            console.log('Cannot start game - room state is:', room.state);
            return;
        }
        ...
        this.broadcastToRoom(room, gameStartMessage);
    }
