...
function diff(oldVNode, newVNode) {
  if (!oldVNode || !newVNode) {
    console.warn('Invalid VNodes in diff:', { oldVNode, newVNode });
    return;
  }

  const parentEl = oldVNode.ref;
  if (!parentEl) {
    console.warn('Parent element not found for VNode:', oldVNode);
    return;
  }
  ...
  for (let i = 0; i < newChildren.length; i++) {
    ...
    if (matchIndex !== -1) {
      ...
      if (typeof newChild !== "boolean" && newChild !== null) {
  ...
  oldChildren.forEach((oldChild, index) => {
    if (!matchedOld.has(index) && oldChild?.ref) {
      oldChild.ref.remove();
    }
  });
  ...
  if (currentComponent && componentStates.has(currentComponent)) {
